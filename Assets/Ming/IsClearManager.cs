...
public class IsClearManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if(PlayerPrefs.GetInt("FinalCutScene", 0) == 0 && PlayerPrefs.GetInt(maximumChapter+"-"+stagesPerChapter, 0) > 0)
        {
            GameObject.Instantiate<GameObject>(finalCutScene, transform);
