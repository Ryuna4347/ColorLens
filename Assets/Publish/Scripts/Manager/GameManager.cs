...
public class GameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (!SceneManager.GetActiveScene().name.Equals("1-1") && !SceneManager.GetActiveScene().name.Equals("2-8")) //컷신 존재시 튜토리얼 UI는 잠시 미룬다.
        {
            CheckTutorial();
        }
        ...
        touchUI = GameObject.Instantiate(touchPrefab, GameObject.Find("GameCanvas").transform);
#endif
    ...
    private void ReadMapData()
    {
        ...
        for (i = 0; i < obstacleMap.Count; i++)
        {
            if (obstacleMap[i].transform.childCount == 2) //렌즈의 경우 2칸을 차지하고 있으므로 별도 처리를 한다.
            {
                ...
                objPos.y = (float)Math.Round(objPos.y - 0.5f) + (height / 2);
            ...
            else
            {
                ...
                objPos.x = Mathf.Ceil(objPos.x) + (width / 2) - 1;
                objPos.y = objPos.y + (height / 2) - 0.5f;
    ...
    public List<GameObject> GetObjsNextPosition(string characterName, Vector2 characterPos, Direction dir)
    {
        ...
        {
            Debug.LogError("게임 정보가 일치하지 않습니다.(잘못된 캐릭터) " + characterName);
            return null;
        }
    ...
    private int[] ConvertPosToTwoDimentionIdx(Vector2 pos)
    {
        ...
        index[1] = (int)(pos.y + (height / 2) - 0.5f);
    ...
    private IEnumerator CheckGameOver()
    {
        while (!isGameOver)
        {
            ...
            { //게임 클리어
                ...
                if (SceneManager.GetActiveScene().name == "7-12" && PlayerPrefs.GetInt("FinalCutScene", 0) < 1)
                {
                    yield return new WaitForSeconds(0.5f);
                    ClearEffect.instance.stars = star;
                    ClearEffect.instance.walkCount = moveCount;
                    Image finalCutSceneImage = GameObject.Find("FinalCutScene").GetComponent<Image>();
                    finalCutSceneImage.raycastTarget = true;
                    Color color = finalCutSceneImage.color;
                    color.a = 1;
                    finalCutSceneImage.color = color;
                }
                else
                {
    ...
    private IEnumerator CheckCharactersMove()
    {
        ...
        yield return new WaitForSeconds(0.05f / moveRatio); // 모든 캐릭터가 멈춰있어도 일정 시간 다음 이동에 딜레이를 준다.
    ...
    public bool CharacterMerge(List<GameObject> colors)
    {
        ...
        {
            string firstColor = colors[0].name.Split('_')[0];
            string secondColor = colors[1].name.Split('_')[0];

            List<string> firstColorList = CommonFunc.GetColorCombination(firstColor);
            List<string> secondColorList = CommonFunc.GetColorCombination(secondColor);
            string mergeColor = CommonFunc.GetColorName(firstColorList.Union(secondColorList).ToList());

            if(firstColor != mergeColor && secondColor != mergeColor)
                mergeChar = unusedCharacters.Find(x => x.name.Contains(mergeColor));
            else
            {
                if (firstColor == mergeColor)
                    colors.Remove(colors[0]);
                if (secondColor == mergeColor)
            ...
            if (mergeChar != null && (firstColor != mergeColor && secondColor != mergeColor))
            {
                mergeChar.SetActive(true);
                mergeChar.transform.position = mergePos;
                UpdateCharacterActive(mergeChar, mergePos, true);
    ...
    public bool CharacterFilter(PlayerMove1 character, ColorFilter filter)
    {
        ...
        if (filteredColorName != null && filteredColorName != "")
        {
            ...
            if (moveRatioChanged != null)
                moveRatioChanged(moveRatio);
            filteredCharacter.GetComponent<PlayerMove1>().CalculateRoute(dir, dir);
        }
        return true;
    }
            ...
            for (int j = 0; j < width; j++)
            {
                if (characterArr[i, j].Count >= 1)
                {
                    foreach (GameObject obj in characterArr[i, j])
                    {
                        ...
                        sphere.transform.position = new Vector3(j - (width / 2) + 0.5f, i - height / 2 + 0.5f, -2f);
                    }
                }
            }
        }
